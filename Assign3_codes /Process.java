...
public class Process extends Thread {
	...
	// config is config file (to pass to kernel.init())
	{
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				else if (command.equals("READSEQ") || command.equals("WRITESEQ"))
	...
	// setup
	{
		...
		controlPanel.timeValueLabel.setText(Integer.toString(kernel.clock) + " (ns)");
	...
	// public void run() // add 1/2 second between steps
	// {
	// haveDelay = true;
	// while (runs < runcycles) {
	// step();
	// System.out.println(runcycles);
	...
	private void runReadWriteCommand(Instruction instruct) {
		...
		controlPanel.addressValueLabel.setText(Long.toString(instruct.dataAdr, kernel.addressradix));
	...
	private void runSeqCommand(Instruction instruct) {
		...
		kernel.logMessage("-------------------------------------");
		kernel.logMessage("Running " + instruct.inst + " " + "Num " + instruct.loopNum + " " + "CdAdr "
				+ instruct.codeAdr + " " + "DtAdr " + instruct.dataAdr + " " + "Inc " + instruct.increment);
		for (i = 0; i < instruct.loopNum; i++) {
			// Read code
			controlPanel.instructionValueLabel.setText("READ");
			controlPanel.addressValueLabel.setText(Long.toString(instruct.codeAdr, kernel.addressradix));
			...
			if (instruct.inst.equals("READSEQ")) {
				controlPanel.instructionValueLabel.setText("READ");
				controlPanel.addressValueLabel.setText(Long.toString(dtAdr, kernel.addressradix));
		...
		kernel.logMessage("Completed " + instruct.inst + " " + "Num " + instruct.loopNum + " " + "CdAdr "
				+ instruct.codeAdr + " " + "DtAdr " + instruct.dataAdr + " " + "Inc " + instruct.increment);
		kernel.logMessage("-------------------------------------");
	}
	...
	private void runLoopCommand(Instruction instruct) {
		...
		kernel.logMessage("-------------------------------------");
		kernel.logMessage("Running " + instruct.inst + " " + "Num " + instruct.loopNum + " " + "CdAdr "
				+ instruct.codeAdr + " " + "AdrLst " + dtadrs);
		for (i = 0; i < instruct.loopNum; i++) // run specified times
		{
			...
			// address
			{
				...
				// Read data
				controlPanel.instructionValueLabel.setText("WRITE");
				controlPanel.addressValueLabel.setText(Long.toString(instruct.adrList[j], kernel.addressradix));
